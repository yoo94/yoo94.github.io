...
- HTTP헤더
- 웹보안
keywords: CORS, Cross-Origin Resource Sharing, Same-Origin Policy, 프리플라이트 요청, Credentialed Request, XMLHttpRequest, Access-Control-Allow-Origin, Access-Control-Allow-Credentials, Spring Boot CORS 설정, @CrossOrigin, WebMvcConfigurer, HTTP 메서드, 보안 정책, 교차 출처 요청
...
![이미지](/blog/postImg/Pasted image 20240202165833.png)
...
![이미지](/blog/postImg/Pasted image 20240202170012.png)

####### 요청하는 클라이언트와 요청받는 서버가 **같은 출처에 있으면 동일 출처**, **서로 다른 서버에 있으면 다른 출처 요청**
![이미지](/blog/postImg/Pasted image 20240202170045.png)
...
xhr.send();
```
![이미지](/blog/postImg/Pasted image 20240202170146.png)
...
xhr.send();
```
![이미지](/blog/postImg/Pasted image 20240202170258.png)
